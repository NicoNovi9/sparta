...
#PBS -q gpu
#PBS -l select=1:ncpus=1:mpiprocs=1:mem=250GB:ngpus=1:gpu_type=h200
#PBS -l walltime=02:00:00
#PBS -j oe

# Nsight Systems profile + post-processing in one job, on one H200.
# Copy of submit_gpu_nsys_h200.sh with H200 resources and defaults: by default it
# reruns the 30M-particle case that failed with BadAlloc on
# collide:nn_last_partner, now built against Kokkos 5.2.2.
...
# ---------------------------------------------------------------- paths ----
# Submit from inside this directory:   cd nicola && qsub submit_gpu_nsys_h200.sh
...
#   qsub -v CASE_DIR=/path/to/case submit_gpu_nsys_h200.sh
...
#   qsub -v NSYS_DELAY=120,NSYS_DURATION=60 submit_gpu_nsys_h200.sh
...
for p in "$CASE_DIR" "$SPARTA_EXE" "$INPUT_FILE"; do
    if [ ! -e "$p" ]; then
        echo "MISSING: $p" >&2
        echo "override with:  qsub -v CASE_DIR=...,SPARTA_EXE=... submit_gpu_nsys_h200.sh" >&2
        exit 1
    fi
done
